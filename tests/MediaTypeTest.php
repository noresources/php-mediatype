...
class MediaTypeTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testParse()
    {
        ...
        foreach ($tests as $text => $parsed) {
            $mediaType = null;
            try {
                $mediaType = MediaTypeFactory::getInstance()->createFromString($text, $parsed['class'] == MediaRange::class);
            ...
            $this->assertEquals(Container::keyValue($parsed, 'string', $text), \strval($mediaType), $text . ' to string');

            $this->assertEquals(Container::keyValue($parsed, 'serialized', $text), $mediaType->jsonSerialize(), $text . ' (re-)serialized');

            if ($parsed['subtype']) {
                $this->assertInstanceOf(MediaSubType::class, $mediaType->getSubType(), $text . ' subtype');

                $subType = $mediaType->getSubType();

                $this->assertCount(count($parsed['subtype']['facets']), $subType->getFacets(), $text . ' subtype facets');

                $this->assertEquals(Container::keyValue($parsed['subtype'], 'syntax'), $subType->getStructuredSyntax(), $text . ' subtype syntax');

                foreach ($parsed['subtype']['facets'] as $index => $facet) {
                    $this->assertEquals($facet, $subType->getFacet($index), $text . ' subtype facet ' . $index);
                }
            } else
                $this->assertEquals(MediaRange::ANY, $mediaType->getSubType(), 'Subtype is a range');

            $toleranceFlags = Container::keyValue($parsed, 'toleranceFlags', 0);
            $this->assertEquals(Container::keyValue($parsed, 'syntax'), $mediaType->getStructuredSyntax($toleranceFlags), $text . ' syntax');
        }
    }
    ...
    public function testParseParameters()
    {
        ...
        foreach ($tests as $input => $expected) {
            $mediaType = MediaTypeFactory::getInstance()->createFromString($input);
    ...
    public function testFromMedia()
    {
        $tests = [
            ...
            'data:// stream' => [
                'media' => $this->addStream(\fopen('data://text/csv,foo,bar', 'r')),
                'expected' => 'text/csv'
            ]
        ];
        ...
        foreach ($tests as $label => $test) {
            ...
            $message = '';
            try {
                $mediaType = MediaTypeFactory::getInstance()->createFromMedia($media, MediaTypeFactory::FROM_ALL);
            } catch (\Exception $e) {
        ...
        $mode = MediaTypeFactory::FROM_ALL | MediaTypeFactory::FROM_EXTENSION_FIRST;

        $this->assertStringEndsWith('/javascript', strval(MediaTypeFactory::getInstance()->createFromMedia(__DIR__ . '/data/c++.js', $mode)), 'C++ code in a .js file (extension first)');
    }
    ...
    public function testCompareSubTypes()
    {
        ...
        foreach ($tests as $label => $test) {
            ...
            $actual = null;
            $expected = $test[2];
            $label = $test[0] . ' < ' . $test[1] . ' = ' . $expected . ' (' . $label . ')';
            try {
                $a = MediaRange::createFromString('whatever/' . $test[0]);
                $a = $a->getSubType();
                $b = MediaRange::createFromString('whatever/' . $test[1]);
                $b = $b->getSubType();
    ...
    public function testCompareRanges()
    {
        ...
        foreach ($tests as $test) {
            ...
            $this->assertEquals($expected, $actual, $label . ' = ' . $expected);
    ...
    public function testLexicalCompare()
    {
        ...
        foreach ($tests as $label => $test) {
            ...
            elseif ($actual > 0)
                $actual = 1;
            $this->assertEquals($expected, $actual, $label . '(reversed)');
        }
    }
    ...
    public function testToString()
    {
        $tests = [
            ...
            [
                'application/vnd.special+json',
                MediaTypeInterface::PART_MAINTYPE | MediaTypeInterface::PART_SYNTAX_SUFFIX,
                'application+json'
            ],
            'Subtype facets and syntax suffix' => [
                'application/vnd.special+json',
                MediaTypeInterface::PART_SUBTYPE_FACETS | MediaTypeInterface::PART_SYNTAX_SUFFIX,
                'vnd.special+json'
            ],
            [
                'a/b; key=value',
        ...
        foreach ($tests as $label => $test) {
            $mediaType = MediaTypeFactory::getInstance()->createFromString($test[0]);
            $partFlags = $test[1];
            $expected = $test[2];
    ...
    public function testMatch()
    {
        ...
        foreach ($tests as $label => $test) {
            ...
            $actual = $a->match($b);
            $this->assertEquals($expected, $actual, $label . ' ' . \strval($a) . ' vs ' . \strval($b));
        }
    }
    ...
    public function testRegistry()
    {
        ...
        foreach ([
            ...
            $this->assertEquals($registered, $asString, $type . ' (string) is ' . ($registered ? 'not ' : '') . 'registered');
            try {
            ...
            if ($type instanceof MediaTypeInterface) {
                ...
                $this->assertEquals($registered, $asString, \strval($type) . ' (MediaType) is ' . ($registered ? 'not ' : '') . 'registered');
            }
        }
    }
    ...
    public function testClone()
    {
        ...
        $this->assertEquals($sa, $a->toString(), 'Clone modification does not affect original parameter map');
        $a->getParameters()->offsetSet('foo', 'bar');
        $this->assertEquals($sb, $b->toString(), 'Source modification does not affect clone parameter map');
    }
